/* Drag-gizmo editor for aligning the 3D building models — the TransformControls
   ...
   gizmo. The editor is button-driven: Move (G) / Rotate (R) pick
   ...
   scale, z_offset relative to the terrain at the new spot), and a horizontal
   move keeps the building glued to the terrain. */
(function attachBuildingEditor(global) {
  ...
  function create(viewer) {
    ...
    function setMode(mode) {
      ...
      state.controls.setMode(mode === 'rotate' ? 'rotate' : 'translate');
    ...
    function ensureControls() {
      ...
      c.addEventListener('dragging-changed', (event) => {
        ...
        if (event.value) {
          c.userData.drag = state.selected && c.mode === 'translate'
            ? { axis: c.axis, z_offset: entryOf(state.selected).placement.z_offset }
            : null;
        } else {
