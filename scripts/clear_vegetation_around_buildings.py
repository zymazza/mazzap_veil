...
def process(instances, rings, grid, crown_aware):
    kept = []
    moved = deleted = 0
        ...
        for _sweep in range(8):
            ...
            for bid, ring, margin in rings:
                ...
                if inside or d < clearance:
                    ...
                    inst['x'] = round(nx, 3)
                    inst['y'] = round(ny, 3)
                    # sample_height already returns the absolute terrain elevation
                    # (grid['heights'] are absolute, matching analyze_vegetation.py's
                    # z convention). Do NOT add minElevation — that double-counts the
                    # datum and was storing relocated stems ~minElevation metres high.
                    inst['z'] = round(sample_height(grid, nx, ny), 2)
                    x, y = nx, ny
                    swept = 'move'
            if swept == 'delete':
                action = 'delete'
...
def main():
    ...
    for kind, layer, attrs, crown_aware in [
        ...
        instances = store.instances(kind, layer, 'member_parcel', attrs,
                                    include_id=False)
        kept, moved, deleted = process(instances, rings, grid, crown_aware)
